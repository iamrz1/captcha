...
var (
	// GCLimitNumber The number of captcha created that triggers garbage collection used by default store.
	GCLimitNumber = 99999
	// Expiration time of captcha used by default store.
	Expiration = 1 * time.Minute
	// DefaultMemStore is a shared storage for captcha, generated by New function.
	store = base64Captcha.NewMemoryStore(GCLimitNumber, Expiration)
)
...
func generateCaptchaHandler(w http.ResponseWriter, r *http.Request) {
	...
	if os.Getenv("ENV") != "prod"{
		meta = map[string]string{"answer": answer}
	}

	serveJSONObject(w, http.StatusOK, "Captcha generated", data, meta, true)
}

// captchaVerifyHandler verify http handler
